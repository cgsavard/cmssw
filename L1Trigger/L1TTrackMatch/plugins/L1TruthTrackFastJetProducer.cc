...
void L1TruthTrackFastJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (iterL1Track = TTTrackHandle->begin(); iterL1Track != TTTrackHandle->end(); iterL1Track++) {
    ...
    if (trk_nPS < trkNPSStubMin_)
      continue;
    if ((std::abs(recoVtx - trk_z0) > deltaZ0Cut_) && minBias_)
      continue;
    ...
    if (tp_eventid > 0 && !minBias_) // matched tp is from pileup
      continue;
...
//void L1TruthTrackFastJetProducer::beginJob() {}

//void L1TruthTrackFastJetProducer::endJob() {}
